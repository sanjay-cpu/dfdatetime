...
then
	...
	docker exec "${CONTAINER_NAME}" sh -c "git clone https://github.com/log2timeline/dfdatetime.git";

	if test ${TRAVIS_PYTHON_VERSION} = "2.7";
	then
		docker exec "${CONTAINER_NAME}" sh -c "cd dfdatetime && python2 run_tests.py";
	else
		docker exec "${CONTAINER_NAME}" sh -c "cd dfdatetime && python3 run_tests.py";
	fi
